...
class NODE:
	...
	def out_conll(self,feats=False):
		...
		if feats:
			...
			for tok in self.tokens:
				if tok.head == "0" and not tok.func == "punct":
					head_word = "head_tok="+tok.lemma.decode("utf-8")
			...
			if self.tokens[0].text.istitle():
				...
				caps = "nocaps"
			last_tok = self.tokens[-1].lemma.decode("utf-8")
		...
		return "\t".join([self.id, snippet, "_","_", "_", feats, self.dep_parent, self.dep_rel, "_", "_"])

	def out_malt(self):
		first = self.tokens[0].lemma.decode("utf-8")
		first_pos = self.tokens[0].pos
		self.rebuild_parse()
		head_word = "_"
		for tok in self.tokens:
			if tok.head == "0" and not tok.func == "punct":
				head_word = tok.lemma.decode("utf-8")
				head_func = tok.func
				head_pos = tok.pos
			if tok.pos in ["PRP", "PP"]:
				pro = "pro"
		...
		return "\t".join([self.id, first, head_word, self.s_type, first_pos, feats, self.dep_parent, self.dep_rel, "_", self.parse.decode("utf-8")])
...
for rstfile in files:
	...
	if dep_root != "":
		...
		conll_tokens = get_tok_info(ntpath.basename(out_file).replace(".rsd",""),corpus_root)
		feats = True
	else:
		feats = False
	...
	for nid in nodes:
		...
		elif nodes[node.parent].kind == "multinuc" and nodes[node.parent].leftmost_child == nid:
			new_rel = get_nonspan_rel(nodes, node)
	...
	for node in out_graph:
		if out_mode == "conll":
			out_handle.write(node.out_conll(feats=feats).encode("utf-8") + "\n")
		else:
			out_handle.write(node.out_malt().encode("utf-8") + "\n")
	out_handle.write("\n")
	out_handle.close()
